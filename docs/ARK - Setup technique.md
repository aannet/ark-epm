...
│   └── schema.sql              ← schéma PostgreSQL versionné (v0.4) — référence SQL des triggers
...
> 💡 `docs/schema.sql` reste la référence pour les triggers PostgreSQL (audit trail), qui sont hors périmètre de Prisma. Lors du premier déploiement, les triggers sont appliqués manuellement ou via un script d'init séparé.
...
|---|---|---|
|**1**|✅ Définir `docs/schema.sql` — 6 objets P1 + auto-référence Business Capabilities + tables de liaison n:n + entité Domaine + audit trail par triggers|Fait. schema.sql v0.4 stable.|
|**2**|Définir `docs/openapi.yaml` — contrat API complet avant de coder|API-first. OpenCode génère un meilleur code NestJS à partir d'un contrat OpenAPI explicite.|
