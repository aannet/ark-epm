...
| F-01 | **Design System & UI Foundation** — Theme MUI, Layout Shell (Sidebar/TopBar), composants partagés, `NotFoundPage` (404), `ErrorBoundary` | F-00 | `draft` | 1j |

> F-00 et F-01 sont réalisés manuellement. Pas de génération OpenCode — ce sont les socles sur lesquels toutes les specs s'appuient. F-01 doit être terminé avant toute tâche frontend dans FS-01.

> **Composants disponibles après F-01 :** `AppShell`, `Sidebar`, `TopBar`, `PageContainer`, `StatusChip`, `ConfirmDialog`, `EmptyState`, `PageHeader`, `LoadingSkeleton`, `NotFoundPage`, `ErrorBoundary`. Ces composants sont réutilisés dans toutes les Feature-Specs suivantes — ne pas les recréer.
...
| ID | Feature | Dépend de | Statut | Estimé |
|---|---|---|---|---|
| FS-01 | **Auth & RBAC** — Login email/password, JWT, gestion des rôles et permissions, guards NestJS, écran login React, pages `UnauthorizedPage` (401) et `ForbiddenPage` (403) | F-00, F-01 | `draft` | 3j |
...
- [ ] F-01 terminé (theme MUI actif, AppShell fonctionnel, composants partagés disponibles, `NotFoundPage` et `ErrorBoundary` en place)
