...
- [x] **F-02 `done`** — `useTranslation()` available ✅
- [x] **F-03 `done`** — `DimensionTagInput` + `TagChipList` available ✅
- [ ] **i18n keys added** — 55 `data-objects.*` keys in `fr.json`
- [x] **`hasPermission()` exported** — from `@/store/auth` ✅
- [ ] **App.tsx wired manually** — 4 routes + PrivateRoute
- [ ] **Sidebar entry added** — label + icon + href
- [x] **Cypress operational** — `cy.loginAsAdmin()`, `cy.loginAsReadOnly()` ✅
