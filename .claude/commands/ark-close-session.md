...
- `statut` : `open` ou `done`
- `date_resolution` : `~` si open, date ISO `YYYY-MM-DD` si done
- `type` : `spec | decision | review | debt | impl | test | doc | poc`
- `sessions[].tool` : `CL` pour cette session Claude Code
...
2. **Pour chaque tâche existante dans tasks.yaml touchée durant cette session** :
   - Si complétée → passe `statut: done` + renseigne `date_resolution` (date du jour)
   - Ajoute une entrée dans `sessions` : `tool: CL`, `id: "<6 chars hex>"`, `nom: "<description courte>"`
