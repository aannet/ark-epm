...
export default function ITComponentForm({ initialValues, onSubmit, onCancel, isLoading = false, error }: ITComponentFormProps): JSX.Element {
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      <TextField name="name" label={t('it-components.form.nameLabel')} value={values.name} onChange={(e) => setValues(v => ({ ...v, name: e.target.value }))} required fullWidth autoFocus error={!!error} helperText={error === 'duplicate' ? t('it-components.form.nameDuplicate') : error === 'validation' ? t('it-components.form.nameRequired') : ''} disabled={isLoading} slotProps={{ htmlInput: { maxLength: 255 } }} />
      <TextField name="technology" label={t('it-components.form.technologyLabel')} value={values.technology} onChange={(e) => setValues(v => ({ ...v, technology: e.target.value }))} fullWidth placeholder={t('it-components.form.technologyPlaceholder')} disabled={isLoading} slotProps={{ htmlInput: { maxLength: 255 } }} />
      <TextField name="type" label={t('it-components.form.typeLabel')} value={values.type} onChange={(e) => setValues(v => ({ ...v, type: e.target.value }))} fullWidth placeholder={t('it-components.form.typePlaceholder')} disabled={isLoading} slotProps={{ htmlInput: { maxLength: 100 } }} />
      <TextField name="description" label={t('it-components.form.descriptionLabel')} value={values.description} onChange={(e) => setValues(v => ({ ...v, description: e.target.value }))} fullWidth multiline rows={3} disabled={isLoading} slotProps={{ htmlInput: { maxLength: 2000 } }} />
      <TextField name="comment" label={t('it-components.form.commentLabel')} value={values.comment} onChange={(e) => setValues(v => ({ ...v, comment: e.target.value }))} fullWidth multiline rows={3} disabled={isLoading} slotProps={{ htmlInput: { maxLength: 2000 } }} />
